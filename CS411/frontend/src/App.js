    ...
    switch (type.name) {
      case "Weather":
        ...
        const { data: weather } = await axios.get(
          `http://api.openweathermap.org/data/2.5/weather?lat=${location.lat}&lon=${location.lon}&appid=537275397b9037ad50e8da9814692add`
        );
        setWeatherInfoDes(weather?.weather[0]?.description);
        setWeatherInfoTem(weather?.main?.temp);
        setVisible(true);
        break;
      default:
        break;
